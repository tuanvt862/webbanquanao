...
namespace MarkShop.Controllers
{
    public class ThemXoaSuaController : Controller
    {
        ...
        public ActionResult QuanLiDonHang()
        {
            ...
            var loadData = db.HoaDons.OrderBy(n => n.MaHD);
            var check = db.ChiTietHoaDons.Count();
            ViewBag.TongDoanhThu = TongDoanhThu();
            return View(loadData);
        }
        [HttpPost]
        public ActionResult ViewDetailCart(int id)
        {
            var qr = db.ChiTietHoaDons.Where(n => n.MaHD.Equals(id)).ToList();
            if (qr == null)
            {
                return Json(new { statusCode = 404, message = "Không tồn tại id đơn hàng!" });
            }
